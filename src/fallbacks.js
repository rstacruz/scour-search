...
fallbacks['$ne'] = binaryOperator((a, b) => a !== b)
...
fallbacks['$where'] = binaryOperator((a, b, key) => b(a, key))
fallbacks['$size'] = binaryOperator((a, b, key) => a.length === b)

function binaryOperator (fn) {
  return function ({ key, value }) {
    key = normalizeKeypath(key)
    return (item, _key) => fn(get(item, key), value, _key)
  }
}
...
fallbacks['$nor'] = function ({ key, value }, build) {
  const fns = value.map(build)
